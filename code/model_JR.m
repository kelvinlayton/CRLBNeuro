...
% Jan 2013
%
function out = model_JR(x,mode,params)
...
% Sigmoid functions
%
f_p = 1 / (1 + exp(r*(v_0 - (x(3)-x(5)))));   % pyramidal firing
f_e = 1 / (1 + exp(r*(v_0 - C1*x(1))));       % excitatory feedback firing
f_i = 1 / (1 + exp(r*(v_0 - C3*x(1))));       % inhibitory feedback firing
...
if mode(1)=='t';
    ...
    % Linearise g()
    %
    f_p_derivative = 2*e_0*r*f_p*(1-f_p);         % pyramidal firing
    f_e_derivative = 2*e_0*r*C1*f_e*(1-f_e);      % excitatory feedback firing
    f_i_derivative = 2*e_0*r*C3*f_i*(1-f_i);      % inhibitory feedback firing
